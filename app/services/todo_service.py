...
class TodoService:
    async def add_todo(self, db: AsyncSession, user_id: int, text: str) -> str:
        await crud.create_todo(db, user_id, text)
        return f"Todo added: {text}"

    async def list_todos(self, db: AsyncSession, user_id: int) -> str:
    ...
    async def mark_done(self, db: AsyncSession, user_id: int, todo_id: int) -> str:
